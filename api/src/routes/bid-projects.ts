/** bid_projects CRUD — 입찰 프로젝트. 호출 8회. */
...
const CreateBody = z.object({
  ...
  bid_number: z.string().min(1).max(50),
  lot_id: z.string().uuid().optional(),
  bid_type: z.string().min(1).max(30),
  contract_type: z.string().min(1).max(30),
  category: z.string().max(100).optional(),
  estimated_amount: z.number().int().nonnegative().optional(),
  design_amount: z.number().int().nonnegative().optional(),
  description: z.string().optional(),
  status: z.string().max(20).default("draft"),
  assigned_to: z.string().uuid().optional(),
}).passthrough(); // 너무 많은 컬럼이 있으니 선택 컬럼은 그냥 통과

const UpdateBody = CreateBody.partial();
const IdParam = z.object({ id: z.string().uuid() });

export async function registerBidProjectsRoutes(app: FastifyInstance) {
  app.get(PATH, { preHandler: [app.authenticate] }, async (req) => {
    const q = ListQuery.parse(req.query);
    const wb = new WhereBuilder()
      .eq("status", q.status)
      .eq("bid_type", q.bid_type)
      .eq("lot_id", q.lot_id)
      .ilikeAny(["title", "bid_number", "category"], q.q);
  ...
  app.get<{ Params: { id: string } }>(`${PATH}/:id`, { preHandler: [app.authenticate] }, async (req) => {
    ...
    return fetchOne(TABLE, id, "입찰을 찾을 수 없습니다.");
  });
  ...
  app.patch<{ Params: { id: string } }>(
    ...
    async (req) => {
      ...
      const before = await fetchOne(TABLE, id, "입찰을 찾을 수 없습니다.");
