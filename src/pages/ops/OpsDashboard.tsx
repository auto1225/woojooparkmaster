...
export default function OpsDashboardPage() {
  ...
  const { data: staff } = useQuery({ queryKey: ["ops-staff-count"], queryFn: async () => {
    const { count, error } = await supabase.from("operations_staff").select("*", { count: "exact", head: true }).eq("is_active", true);
    if (error) throw error;
    return count || 0;
  }});

  const { data: contracts } = useQuery({ queryKey: ["ops-contracts"], queryFn: async () => {
    const { data, error } = await supabase.from("outsourcing_contracts").select("*, parking_lots(code, name)").eq("status", "active");
    if (error) throw error;
    return data || [];
  }});

  const { data: passes } = useQuery({ queryKey: ["ops-passes-count"], queryFn: async () => {
    const { count, error } = await supabase.from("monthly_passes").select("*", { count: "exact", head: true }).eq("status", "active");
    if (error) throw error;
    return count || 0;
  }});

  const { data: enforcement } = useQuery({ queryKey: ["ops-enforcement-unpaid-recent"], queryFn: async () => {
    const { data, error } = await supabase.from("enforcement_records").select("*, parking_lots(code, name)").in("payment_status", ["unpaid", "overdue"]).order("violation_date", { ascending: false }).limit(10);
    if (error) throw error;
    return data || [];
  }});

  const { data: unpaidCount = 0 } = useQuery({ queryKey: ["ops-enforcement-unpaid-count"], queryFn: async () => {
    const { count, error } = await supabase.from("enforcement_records").select("*", { count: "exact", head: true }).in("payment_status", ["unpaid", "overdue"]);
    if (error) throw error;
    return count || 0;
  }});
  ...
  const { data: expiringPasses } = useQuery({ queryKey: ["ops-expiring-passes"], queryFn: async () => {
    ...
    const { data, error } = await supabase.from("monthly_passes").select("*, parking_lots(name)").eq("status", "active").gte("pass_end", today).lte("pass_end", d7.toISOString().split("T")[0]).order("pass_end").limit(5);
    if (error) throw error;
    return data || [];
  }});
    ...
    <DashboardLayout>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {kpis.map(k => (
            <Card key={k.label} role="button" tabIndex={0} aria-label={`${k.label} ${k.value}건 보기`} className="cursor-pointer hover:shadow-md transition-shadow focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" onClick={k.onClick} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); k.onClick(); } }}>
