...
//  SEED – 전체 모듈 데모 데이터
// ══════════════════════════════════════════════════════════════════
export async function runDemoSeed(supabase: any, userId: string) {
  // 매번 재생성 전에 기존 데모 데이터 정리
  await runDemoCleanup(supabase);
  ...
  const compRows = Array.from({ length: 30 }, (_, i) => {
    ...
    return {
      ...
      is_repeat: i % 7 === 0,
      repeat_count: i % 7 === 0 ? 2 + (i % 3) : 0,
      saeol_ref: i % 5 === 0 ? `SAEOL-DEMO-${String(i + 1).padStart(4, "0")}` : null,
      satisfaction_score: status === "closed" ? 2 + (i % 4) : null,
      satisfaction_feedback: status === "closed" ? "[DEMO] 처리 결과 확인 및 만족도 응답" : null,
      notes: "[DEMO] 데모 민원",
  ...
  // 실제 보고서 파일은 보고서 센터의 "전체 샘플 새 양식으로 교체"에서 생성한다.
  // 존재하지 않는 저장소 경로를 이력에 넣으면 다운로드가 실패하므로 가짜 보고서 이력은 만들지 않는다.
...
//  CLEANUP – 데모 데이터 삭제
// ══════════════════════════════════════════════════════════════════
export async function runDemoCleanup(supabase: any) {
