...
interface Props {
  ...
  onUnlinked?: (document: OfficialDocument) => Promise<void> | void;
}
...
export function DocumentLinksPanel({
  ...
  const [relationType, setRelationType] =
    useState<DocumentRelationType>("reference");
  ...
  const changeRelation = async (
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey });
      await queryClient.invalidateQueries({
        queryKey: ["official-document-links", link.documentId],
      });
      toast.success("문서 관계를 변경했습니다.");
    } catch (error: any) {
      toast.error("문서 관계 변경에 실패했습니다.", {
        description: error.message,
      });
    }
  };

  const remove = async ({ linkId, document }: PendingDelete) => {
    try {
      await unlinkOfficialDocument(linkId);
      await onUnlinked?.(document);
      await queryClient.invalidateQueries({ queryKey });
      await queryClient.invalidateQueries({
        queryKey: ["official-document-links", document.id],
      });
  ...
  return (
    <Card>
      <CardHeader className="flex flex-col items-stretch gap-3 space-y-0 pb-3 sm:flex-row sm:items-center sm:justify-between">
        <CardTitle className="flex items-center gap-2 whitespace-nowrap text-sm">
        ...
        {!readOnly && (
          <div className="grid grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-2 sm:flex">
            ...
            >
              ...
              <SelectContent>
                {Object.entries(DOCUMENT_RELATION_LABELS).map(
                  ([value, label]) => (
        ...
        {isLoading ? (
          ...
          <div className="divide-y">
            {data.map(({ document, link }) => (
              <div
                key={link.id}
                className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 py-2.5 sm:flex sm:gap-3"
              >
                ...
                {readOnly ? (
                  <span className="text-xs text-muted-foreground">
                    {DOCUMENT_RELATION_LABELS[link.relationType]}
                  </span>
                ) : (
                  <>
                    <Select
                      ...
                      onValueChange={(value) =>
                        changeRelation(link, value as DocumentRelationType)
                      }
                    >
                      <SelectTrigger
                        className="col-start-2 h-8 w-full text-xs sm:w-28"
                        aria-label="문서 관계 변경"
                      >
                        <SelectValue />
                      </SelectTrigger>
                    ...
                    <Button
                      ...
                      title="연결 해제"
                      onClick={() =>
                        setPendingDelete({ linkId: link.id, document })
                      }
                    >
      ...
      {!readOnly && (
        <LinkDocumentDialog
          ...
          excludedIds={data.map(({ document }) => document.id)}
        />
      )}
      {!readOnly && (
        <OfficialDocumentDialog
          ...
          initialTitle={recordTitle ? `${recordTitle} 관련 문서` : ""}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>문서 연결을 해제하시겠습니까?</AlertDialogTitle>
            <AlertDialogDescription>
              문서 자체는 삭제하지 않으며 이 업무자료와의 연결만 해제합니다.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
